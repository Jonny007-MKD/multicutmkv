...
function check_dependencies1()
{
        ...
        if ! type gawk >/dev/null 2>&1 ; then
                echo -e "$c_error Please install gawk!"
                pist=1;
        fi
        if ! type bc >/dev/null 2>&1 ; then
                echo -e "$c_error Please install bc!"
                pist=1;
        fi
        if ! type $MEDIAINFO >/dev/null 2>&1 ; then
                echo -e $c_error Please install mediainfo!"
                pist=1;
        fi
...
function check_dependencies2()
{
        ...
        if [ $cutwith == "avidemux" ] && ! type $avidemux > /dev/null 2>&1 ; then
                echo -e "$c_error Please install avidemux!"
                pist=1;
        fi
        if [ $cutwith == "avisplit" ] && ! type avisplit > /dev/null 2>&1 ; then
                echo -e "$c_error Please install 'transcode'!"
                pist=1
        fi
        if [ $cutwith == "smartmkvmerge" ]; then
                if ! type $MKVMERGE > /dev/null 2>&1 ; then
                        echo -e "$c_error Please install mkvtoolnix"
                        pist=1
                fi
                if ! type $FFMSINDEX > /dev/null 2>&1 ; then
                        echo -e "$c_error Please install ffmsindex"
                        pist=1
                fi
                if ! type $X264 > /dev/null 2>&1 ; then
                        echo -e "$c_error Please install x264"
                        pist=1
                fi
        fi
...
{ # array-index
        ...
        if [ ! -f "${cutlist[$idx]}" ]; then
                echo -e "$c_error Dowloading cutlist failed!"
                cleanup 1
        fi
...
{ # filename
        ...
        if [ ${#cutlist[@]} -eq 0 ] ; then
                echo -e "$c_error Keine passende Cutlist fuer $c_filename$filename$c_error gefunden. Abbruch.$c_end"
                return 0
        fi
...
function findclosesttime()
{
        ...
        then
                ...
                if [ ! -f "${kflist}" ]; then
                        echo -e "$c_error ffmsindex failed"
                        cleanup 1
                fi
        fi
        echo $millis >> millis
                ...
                then
                        if [ ! -f "${tempdir}/mkv.ok" ]; then
                                ...
                                if [ $? -eq 0 ]; then
                                        ...
                                        echo -e "$c_error mkvmerge failed"
                                        cleanup 1
                                fi
                        fi
...
if [ ! -d "$tempdir" ] ; then
        echo -e "$c_error Temporaerer Ordner $tempdir nicht gefunden!$c_end"
        exit 1
fi
