...
#X264="/home/erika/nobackup/otr-verwaltung-smartmkvmerge/data/tools/intern-x264"
X264=x264
AVCONV=avconv
MKVMERGE=mkvmerge
FFMSINDEX=ffmsindex
...
{ # filename
  ...
  shopt -s compat31
  # x264 arbeitet nur über ffms genau genug nutze den ffms-demuxer oder leb
  # mit ungenauen Schnitten.
...
function findclosesttime()
{
  ...
  if [ ! -f "${kflist}" ]
  then
    $FFMSINDEX -k -p -c -f $film ${film##*/}.ffindex
  fi
  echo $millis >> millis
...
{ # film, cutlist, vcf2format?
  ...
  then
    ...
    then
      $MKVMERGE -o "${tempdir}/$(basename $film).mkv" "$film"
    ...
    if echo $line | grep -q "Start" ; then      ######### startcut ##########
      ...
      if [ $cutwith == "avisplit" ] ; then
        ...
        if type bc > /dev/null ; then
          ...
          echo "please install bc"
          cleanup 1
          # frames=$(( ${length%%.*}*$fps + ${decimalsecs:-0}/4))
        fi
        ...
        elif [ $cutwith == "smartmkvmerge" ] || [ $cutwith == "smartmkvmergeavconv" ]
        then
          ...
          if iskeyframe $sframe $film
          then
            echo start ist keyframe
            if iskeyframe $((sframe + frames)) $film
            then
              ...
              # das häppchen startet mit keyframe aber endet nicht auf
              # keyframe
              ...
              then
                ...
                $AVCONV -ss $preseek -i "${film}"  -ss $avstime $avconvopts -vframes $((frames-ulkeyframe+sframe))   ${outputfilename}.avi
                $MKVMERGE -o "${outputfilename}" "${outputfilename}.avi"
              fi
            ...
            then
              ...
              avstime=$(echo ${avstime} - $preseek| bc -l )
              $AVCONV -ss $preseek -i "${film}"  -ss $avstime $avconvopts -vframes $((eframes)) ${outputfilename}.avi
              $MKVMERGE -o "${outputfilename}" "${outputfilename}.avi"
            fi
            ...
            then
              ...
              then
                ...
                then
                  ...
                  $AVCONV -ss $preseek -i "${film}"  -ss $avstime $avconvopts  -vframes $((sframe+frames-beframe)) ${outputfilename}.avi
                  $MKVMERGE -o "${outputfilename}" "${outputfilename}.avi"
                fi
                video_files[${#video_files[@]}]=$outputfilename
              fi
  ...
  if [ $cutwith == "smartmkvmerge" ] || [ $cutwith == "smartmkvmergeavconv" ]
  then
    ...
    $MKVMERGE --ui-language en_US --split parts-frames:$video_splitframes $mkvmergeopts -o video_copy.mkv $film
    ...
    [[ $cutwith == "smartmkvmerge" ]] &&
    {
      ...
      $MKVMERGE --ui-language en_US -D --split parts:$audio_timecodes -o  audio_copy.mkv $film
      afiles=audio_copy*.mkv
      mkvmergeopts="-D"
    }
    # mux
    $MKVMERGE --engage no_cue_duration --engage no_cue_relative_position --ui-language en_US -o $outname ${vfiles} ${afiles}
    ecode=$?
...
if [ $cutwith == "avidemux" ] && ! type $avidemux > /dev/null ; then
  ...
  exit 1
elif [ $cutwith == "smartmkvmerge" ] && ! type $MKVMERGE && ! type $FFMSINDEX && ! type $X264 > /dev/null ; then
  echo "Bitte installieren Sie die Pakete ffmsindex, x264 und mkvmerge"
  exit 1
fi

# benoetigte ordner (falls nicht vorhanden) erstellen
workdir="$(pwd)"
# temporaeren ordner leeren
...
if [ ! -d "$tempdir" ] ; then
  mkdir -p "$tempdir" ||
  {
    echo "Kann temporaeren Ordner nicht erstellen! Exit."
    exit 1
  }
fi
tempdir=${tempdir%/}
